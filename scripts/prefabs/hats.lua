...
local function MakeHat(name)
	...
	local function _base_onequip(inst, owner, symbol_override, swap_hat_override)
		...
		if inst.components.fueled ~= nil then
			if inst.scrapbook_specialinfo ~= "WATHGRITHRHAT" then
	...
	local function wathgrithr_takefuel(inst)
		inst.components.armor:SetCondition(inst.components.armor.condition + 105)
	end

    fns.wathgrithr = function()
        local inst = simple(wathgrithr_custom_init)
		inst.scrapbook_specialinfo = "WATHGRITHRHAT"
		...
		inst.components.fueled:SetTakeFuelFn(wathgrithr_takefuel)
		inst.components.fueled.accepting = true
